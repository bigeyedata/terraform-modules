...
module "bigeye" {
  source      = "git::https://github.com/bigeyedata/terraform-modules//modules/bigeye?ref=v17.12.1"
  ...
  # AWS IAM Role injection
  ecs_service_role_arn              = aws_iam_role.ecs.arn
  admin_container_ecs_task_role_arn = aws_iam_role.admin_container.arn
  datawatch_task_role_arn           = aws_iam_role.datawatch.arn
  monocle_task_role_arn             = aws_iam_role.monocle.arn
}
